...
def download_and_load_gpt2(model_size, models_dir):
    # Проверяем допустимость размера модели
    ...
    # Загружаем настройки и параметры
...
def download_file(url, destination, backup_url=None):
    ...
    try:
        ...
        if backup_url is not None:
            print(f"Основной URL ({url}) не сработал. Пробуем резервный URL: {backup_url}")
            try:
        ...
        # Если мы дошли до сюда, обе попытки не удались
        error_message = (
            f"Не удалось скачать ни с основного URL ({url})"
            f"{', ни с резервного URL (' + backup_url + ')' if backup_url else ''}."
            "\nПроверьте подключение к интернету или доступность файла.\n"
            "Для получения помощи посетите: https://github.com/rasbt/LLMs-from-scratch/discussions/273"
        )
        print(error_message)
...
def download_file(url, destination):
    ...
    with tqdm(total=file_size, unit="iB", unit_scale=True, desc=progress_bar_description) as progress_bar:
        # Открываем файл назначения в режиме бинарной записи
        with open(destination, "wb") as file:
            # Итерируем по данным файла блоками
            for chunk in response.iter_content(block_size):
                progress_bar.update(len(chunk))  # Обновляем прогресс-бар
                file.write(chunk)  # Записываем блок в файл
"""
...
def load_gpt2_params_from_tf_ckpt(ckpt_path, settings):
    ...
    for name, _ in tf.train.list_variables(ckpt_path):
        # Загружаем переменную и удаляем сингулярные размерности
        variable_array = np.squeeze(tf.train.load_variable(ckpt_path, name))

        # Обрабатываем имя переменной, чтобы извлечь нужные части
